...
def Ustaw_Fromularz_Email(request):
    ...
    if request.session['wybrany_temat'] == '1':
        request.session['poczta_url'] = '/sender/pytanie_o_produkt/'
    ...
    if request.session['wybrany_temat'] == '2':
        request.session['poczta_url'] = '/sender/uwagi_www/'
    ...
    if request.session['wybrany_temat'] == '3':
        request.session['poczta_url'] = '/sender/inny_temat/'
        ...
        # formularz startowy
    request.session['poczta_url'] = '/sender/wybierz_temat/'
    if not request.session['formularz_poczta']:
        request.session['formularz_poczta'] = \
...
def Pytanie_O_Produkt(request):
    ...
    if request.method == 'POST':
        ...
        if formularz.is_valid():
            ...
            email = formularz.cleaned_data['sender']
            produkt = formularz.cleaned_data['product']
            ...
            return redirect('/statement/potwierdzenie/')
